...
app.post('/api/ratings/rate', auth, async (req, res) => {
  ...
  try {
    await client.query(
      `INSERT INTO ratings (user_id, date, rating) VALUES ($1, $2, $3) ON CONFLICT (user_id, date) DO UPDATE SET rating = $3`,
...
app.get('/api/users/search', auth, async (req, res) => {
  ...
  try {
    const result = await client.query(
      `SELECT u.id, u.username, u.avatar, COALESCE(up.points, 0) as points, COALESCE(up.level, 1) as level 
       FROM users u LEFT JOIN user_progress up ON u.id = up.user_id
       WHERE u.username ILIKE $1 AND u.id != $2 LIMIT 20`,
      [`%${q}%`, req.userId]
    );
    res.json(result.rows);
  } finally {
...
app.get('/api/users/:userId/profile', auth, async (req, res) => {
  ...
  try {
    ...
    const statsResult = await client.query('SELECT AVG(rating) as avgRating, COUNT(*) as totalDays FROM ratings WHERE user_id = $1', [userId]);
    res.json({
      user: userResult.rows[0],
      stats: { avgRating: statsResult.rows[0].avgrating || 0, totalDays: statsResult.rows[0].totaldays || 0 }
    });
  } finally {
    client.release();
  }
...
app.get('/api/user/progress', auth, async (req, res) => {
  const client = await pool.connect();
  try {
    let result = await client.query('SELECT points, level, last_rated_date FROM user_progress WHERE user_id = $1', [req.userId]);
